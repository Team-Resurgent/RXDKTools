...
HRESULT DmSetBreakpoint(PVOID addr)
{
	...
	sprintf(sz, "BREAK ADDR=0x%08lx", (unsigned long)(ULONG_PTR)addr);
...
HRESULT DmRemoveBreakpoint(PVOID addr)
{
	...
	sprintf(sz, "BREAK ADDR=0x%08lx CLEAR", (unsigned long)(ULONG_PTR)addr);
	return HrOneLineCmd(sz);
}
...
HRESULT DmIsBreakpoint(PVOID addr, LPDWORD pdwType)
{
    ...
    if(FAILED(hr))
        return hr;
    sprintf(szCmd, "ISBREAK ADDR=0x%08lx", (unsigned long)(ULONG_PTR)addr);
...
HRESULT DmSetDataBreakpoint(PVOID pvAddr, DWORD dwType, DWORD dwSize)
{
	...
	sprintf(sz, "BREAK %s=0x%08lx SIZE=%lu %s", szType, (unsigned long)(ULONG_PTR)pvAddr, dwSize,
		fClear ? "CLEAR" : "");
	return HrOneLineCmd(sz);
}
...
HRESULT DmGetThreadContext(DWORD tid, PXBDM_CONTEXT pcr)
{
	PDM_CONNECTION s;
	HRESULT hr;
	char sz[128];
...
#define FGFpR(reg) pcr->FloatSave.reg = pxfs->reg
				 ...
				 * state */
				XbdmFloatCr0NpxState(&pcr->FloatSave) = dwCr0Npx;
				FGFpR(ControlWord);
				FGFpR(StatusWord);
				FGFpR(TagWord);
				FGFpR(ErrorOffset);
...
HRESULT DmSetThreadContext(DWORD tid, PXBDM_CONTEXT pcr)
{
	PDM_CONNECTION s;
	HRESULT hr;
	char szT[64];
...
#define SFpSR(reg) pxfs->reg = (USHORT)pcr->FloatSave.reg;
		...
		if(pxfs) {
			...
			sprintf(szT, " Cr0NpxState=0x%x", XbdmFloatCr0NpxState(&pcr->FloatSave));
			strcat(sz, szT);
...
#undef SFpSR
	...
	if(pxfs) {
		sprintf(szT, " ext=%d", (int)(sizeof *pxfs));
		strcat(sz, szT);
	}
	hr = DmSendCommand(s, sz, NULL, 0);
...
HRESULT DmGetMemory(const LPBYTE lpbAddr, DWORD cb, LPBYTE lpbBuf, LPDWORD pcbRet)
{
	...
	if(FAILED(hr))
		return hr;
	sprintf(sz, "GETMEM ADDR=0x%08lx LENGTH=0x%08lx", (unsigned long)(ULONG_PTR)lpbAddr, (unsigned long)cb);
	hr = DmSendCommand(s, sz, NULL, 0);
	if(SUCCEEDED(hr) && hr != XBDM_MULTIRESPONSE) {
		TerminateConnection(s);
		hr = E_UNEXPECTED;
...
HRESULT DmSetMemory(const BYTE *lpbAddr, DWORD cb, LPBYTE lpbBuf, LPDWORD pcbRet)
{
	...
	while(cb && SUCCEEDED(hr)) {
		...
		if(cbT > cb)
			cbT = cb;
		sprintf(sz, "setmem addr=0x%08lx data=", (unsigned long)(ULONG_PTR)lpbAddr);
		for(pch = sz; *pch; ++pch);

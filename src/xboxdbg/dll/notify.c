...
void SendNotification(DWORD dwNotification, DWORD_PTR dw)
{
...
void HandleNotification(LPCSTR sz)
{
	...
	if(FGetNotificationData(sz, &dmts.NotifiedReason, &dmts.u)) {
		...
		SendNotification(dmts.NotifiedReason | dwFlags, (DWORD_PTR)&dmts.u);
		...
		if(pchTok)
			dwFlags |= DM_STOPTHREAD;
		SetDmdsSuffixAndLength(sz, &dmds);
		SendNotification(DM_DEBUGSTR | dwFlags, (DWORD_PTR)&dmds);
		...
		FGetDwParam(sz, "start", (DWORD*)&dmct.StartAddress);
		SendNotification(DM_CREATETHREAD | dwFlags, (DWORD_PTR)&dmct);
        ...
        if(FGetDwParam(sz, "start", (PDWORD)&dmfib.StartAddress))
            ...
            return;
        SendNotification(DM_FIBER | dwFlags, (DWORD_PTR)&dmfib);
	}
}

...
LRESULT CALLBACK ExceptionDlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        case 0x80000003:
            ...
            sprintf(szBuf2, "the application at location 0x%08X.", (DWORD)(ULONG_PTR)gs_pvAddress);
            break;

        case 0xC0000005:
            // Access Violation
            sprintf(szBuf1, "The instruction at address 0x%08X referenced memory", (DWORD)(ULONG_PTR)gs_pvAddress);
            if (g_fWriteException)
                sprintf(szBuf2, "at address 0x%08X.  The memory could not be written.", (DWORD)(ULONG_PTR)g_pvAVAddress);
            else
                sprintf(szBuf2, "at address 0x%08X.  The memory could not be read.", (DWORD)(ULONG_PTR)g_pvAVAddress);
            break;

        default:
            // Other
            sprintf(szBuf1, "An exception (0x%08X) occurred in the application", g_dwExceptionCode);
            sprintf(szBuf2, "at location 0x%08X", (DWORD)(ULONG_PTR)gs_pvAddress);
            break;
        }

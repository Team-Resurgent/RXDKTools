...
INT_PTR CALLBACK CPropertyPage::DialogProc(HWND hWndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pThis = (CPropertyPage *)(LONG_PTR)XdkGetDlgUser(hWndDlg);
    if(pThis)
    {
        switch(uMsg)
        {
          ...
          case WM_NOTIFY:
            {
                ...
                if(PSN_APPLY == pPropSheetNotify->hdr.code)
                {
                    ...
                    {
                        XdkSetDlgMsgResult(pThis->m_hPropDlg, PSNRET_INVALID);
                        return TRUE;
                    }
...
INT_PTR CPropertyPage::OnMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    return 0;
}

INT_PTR CPropertyPage::OnCommand(WPARAM wParam, LPARAM lParam)
{
    return 0;
}
...
INT_PTR CVolumeGeneralPage::OnInitDialog(HWND)
/*++
  Routine Descriptor:
...
INT_PTR CFSItemGeneralPage::OnInitDialog(HWND hwndDefaultControl)
{
    ...
    if(IDD_FILEPROP != m_uTemplateResource)
    {
        DWORD dwThreadId;
        m_fKillThread = false;
        m_hThread = CreateThread(NULL, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(CFSItemGeneralPage::StartThreadProc), this, 0, &dwThreadId);
    }

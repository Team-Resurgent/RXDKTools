...
static HRESULT GetColumnSCIDFromTable(PXBOX_COLUMN_HEADER_INFORMATION pTable, UINT uColumnCount, UINT iColumn, SHCOLUMNID *pscid)
{
    if (!pscid || iColumn >= uColumnCount)
    {
        return E_INVALIDARG;
    }
    pscid->fmtid = *pTable[iColumn].pfmtid;
    pscid->pid = pTable[iColumn].pid;
    return S_OK;
}
...
UINT CXboxRoot::GetColumnCount()
{
    return (sizeof(RootColumns)/sizeof(XBOX_COLUMN_HEADER_INFORMATION));
}

HRESULT CXboxRoot::GetColumnSCID(UINT iColumn, SHCOLUMNID *pscid)
{
    return GetColumnSCIDFromTable(RootColumns, GetColumnCount(), iColumn, pscid);
}
...
UINT CXboxConsole::GetColumnCount()
{
    return (sizeof(ConsoleColumns)/sizeof(XBOX_COLUMN_HEADER_INFORMATION));
}

HRESULT CXboxConsole::GetColumnSCID(UINT iColumn, SHCOLUMNID *pscid)
{
    return GetColumnSCIDFromTable(ConsoleColumns, GetColumnCount(), iColumn, pscid);
}
...
UINT CXboxFileSystemFolder::GetColumnCount()
{
    return (sizeof(FileSystemFolderColumns)/sizeof(XBOX_COLUMN_HEADER_INFORMATION));
}

HRESULT CXboxFileSystemFolder::GetColumnSCID(UINT iColumn, SHCOLUMNID *pscid)
{
    return GetColumnSCIDFromTable(FileSystemFolderColumns, GetColumnCount(), iColumn, pscid);
}

...
//
// The DM runtime (xbdbgs.lib) initializes its critical sections / events from a
// CRT static initializer (g_pXboxdbgStaticCtor in xboxdbg_static_init.obj).  That
// object is only reached through the .CRT$XCU table, so when the lib is consumed
// by an EXE the linker would discard it, leaving g_sci and other globals
// uninitialized (an AV on the first DM call).  Force a reference to the
...
typedef DWORD (__stdcall *PDM_NOTIFY_FUNCTION)(ULONG dwNotification, DWORD_PTR dwParam);
...
DMHRAPI DmGetThreadContext(DWORD dwThreadId, PXBDM_CONTEXT pdmcr);
DMHRAPI DmSetThreadContext(DWORD dwThreadId, PXBDM_CONTEXT pdmcr);

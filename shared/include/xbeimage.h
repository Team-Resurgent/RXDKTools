...
// virtual address, independent of the architecture of the host tool that builds
// or reads the image.  Represent these fields with a fixed 32-bit type so that
// the structures keep their exact on-disk layout regardless of whether imagebld
// is compiled as a 32-bit or 64-bit binary.
//
...
typedef struct _XBEIMAGE_SECTION {
    ...
    //
    XBEVA HeadSharedPageReferenceCount;
    XBEVA TailSharedPageReferenceCount;
...
typedef struct _XBEIMAGE_HEADER {
    ...
    // libraries.
    //
    XBEVA XboxKernelLibraryVersion;
    XBEVA XapiLibraryVersion;
...
//
// The XBE structures are an on-disk / on-console format consumed by the 32-bit
// Xbox loader.  Guarantee that building this header into a 64-bit host tool does
// not change the layout: every virtual-address field must stay 4 bytes so the
// structures are byte-identical to a 32-bit build.
//
